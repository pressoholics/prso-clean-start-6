...
class PostRelationSaveHelper
{
     ...
     * @param RelationshipContext $context
     */
    public function syncMetadata(RelationshipContext $context)
    {
        $sourceSiteId = $context->sourceSiteId();
        $remoteSiteId = $context->remoteSiteId();
         ...
         * @param RelationshipContext $context
         */
        $keysToSync = apply_filters(self::FILTER_SYNC_KEYS, [], $context);
    ...
    /**
     * @param RelationshipContext $context
     * @param string $sourceThumbPath
     * @return bool
     */
    public function syncThumb(RelationshipContext $context, string $sourceThumbPath): bool
    {
        $sourceSiteId = $context->sourceSiteId();
        $remoteSiteId = $context->remoteSiteId();
        $remotePostId = $context->remotePostId();

        if ($sourceSiteId === $remoteSiteId || !$remotePostId || !$sourceThumbPath) {
            return true;
        }
        ...
        $filename = wp_unique_filename($uploadPath, basename($sourceThumbPath));

        if (!copy($sourceThumbPath, "{$uploadPath}/{$filename}")) {
            $this->maybeRestoreSite($originalSite);

            return false;
        }

        $filetype = wp_check_filetype("{$uploadUrl}/{$filename}");
        $attachment = [
            'post_mime_type' => $filetype['type'] ?? '',
            'guid' => "{$uploadUrl}/{$filename}",
        ...
        $thumbnailId = wp_insert_attachment($attachment, $fullpath);
        ...
        wp_update_attachment_metadata(
            $thumbnailId,
            wp_generate_attachment_metadata($thumbnailId, $fullpath)
        );

        $result = (bool)update_post_meta($remotePostId, '_thumbnail_id', $thumbnailId);
        $this->maybeRestoreSite($originalSite);
