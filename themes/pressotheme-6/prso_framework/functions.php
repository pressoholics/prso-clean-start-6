...
//							add_image_size( "{$name}--x2", $width, $height, $crop );
	...
	/**
	* default_image_srcset_sizes_attr
	*
	* @CALLED BY FILTER 'wp_calculate_image_sizes'
	*
	* Set the default image srcset sizes attr for all images.
	 * Note this treats the current image size as the desktop, x2 that size for desktop retina, then whatever is suitable for smaller sizes
	*
	* @access public
	* @author Ben Moody
	*/
	public function default_image_srcset_sizes_attr( $srcset_size, $sizes ) {

		$srcset_size = "(min-width: 1024px) $sizes[0]px, (min-width: 2048px) 100vw, 100vw";
	...
	public function insert_custom_image_sizes( $image_sizes ) {
		...
		foreach ( $_wp_additional_image_sizes as $id => $data ) {
			...
			if( !isset($prso_srcset_rules[ $id ][0]['show_in_admin_ui']) ) {
				continue;
			}

			if( true !== $prso_srcset_rules[ $id ][0]['show_in_admin_ui'] ) {
				continue;
			}
