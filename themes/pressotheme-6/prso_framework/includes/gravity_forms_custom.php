...
function prso_theme_gform_form_validation_message( $validation_message, $form ) {
	...
	//Init vars
	$form_validation_msg_classes = 'alert callout';
...
function prso_theme_gform_field_content( $content, $field, $value, $lead_id, $form_id ) {
	...
	if( !is_admin() ) {
			...
			} elseif ( ($field['type'] === 'post_custom_field') || ($field['type'] === 'password') || ($field['type'] === 'radio') || ($field['type'] === 'select') || ($field['type'] === 'fileupload') ) {
...
function prso_theme_gform_get_address_field( $field, $value, $lead_id, $form_id ) {
	...
	if( method_exists( 'GFCommon', 'get_address_types' ) ) {
		$address_types = GFCommon::get_address_types($form_id);
	} else {
		$gf_address_field 	= GF_Fields::get( 'address' );
		$address_types 		= $gf_address_field->get_address_types( $form_id );
	}
...
function prso_theme_gform_get_state_field($field, $id, $field_id, $state_value, $disabled_text, $form_id, $state_label){
    ...
    //Add support for gf versions < 1.9
	if( method_exists( 'GFCommon', 'get_state_dropdown' ) ) {
